...
public class App {
    public static void main(String[] args) throws IOException, ClassNotFoundException {
        ...
        serverSocket.bind(new InetSocketAddress("localhost",12000));
        while(true){
            Socket accept = serverSocket.accept();
            ObjectInputStream input = new ObjectInputStream(accept.getInputStream());
            Request req = (Request) input.readObject();
            Object result = null;
            if(req.getOp().equals("op1")){
                System.out.println("ENCAMINHADO OPERAÇÃO 1 PARA NODE 2 ENCAMINHAR");
                Socket node3 = new Socket("localhost", 11000);
                result = encaminhar(node3, req);
            }else if(req.getOp().equals("op2")){
                System.out.println("ENCAMINHADO OPERAÇÃO 2 PARA NODE 4 RESOLVER");
